...
parser.add_argument("--saved_PSNR_model_path",type=str,default="saved_models/div2k/G_PSNR_20.pth",help="path of saved PSNR model")#输入PSNR模型路径
parser.add_argument("--n_residual_blocks", type=int, default=23, help="number of residual blocks in PSNR generator")
parser.add_argument("--saved_ESRGAN_model_path",type=str,default="saved_models/div2k/G_GAN_20.pth",help="path of saved ESRGAN model")#输入ESRGAN模型路径
...
G_PSNR_meta=RRDBNet_meta(3, 3, 64, opt.n_residual_blocks, gc=32)
...
def save_final_images(path):
    with torch.no_grad():
        ...
        for k in range(10):
            ...
            fake_PSNR_meta=G_PSNR_meta(lr_img,OrderedDict(G_PSNR.named_parameters()))
            ...
            img_res=[r_transform(lr_img.data.cpu()[0]),  #origin image
                        ...
                        r_transform(fake_PSNR_meta.data.cpu()[0]),    #PSNR image
